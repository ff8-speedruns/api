export default function Poles() {
    ...
    if (filteredArray.length > 0)
        response = `${filteredArray[0].code} (${filteredArray[0].input}) -> BACKUP ${filteredArray[0].backup.code} (${filteredArray[0].backup.input})`;
    else
        response = `Code not found D:`;
...
// Build a table to find the Caraway House number
function makeCarawayCodeTable(from, to) {
    ...
    rngStateArr = rngStateArr.map(() => rng.NextRng());
